...
typedef struct _SYSTEM_PROCESS_INFORMATION2_1 {
...
} SYSTEM_PROCESS_INFORMATION2_1;

typedef struct _SYSTEM_THREAD_INFORMATION_1 {
	FILETIME KernelTime;
	FILETIME UserTime;
	FILETIME CreateTime;
	DWORD WaitTime;
...
static bool thread_is_suspended(DWORD process_id, DWORD thread_id)
{
	...
	SYSTEM_PROCESS_INFORMATION2_1*spi = (SYSTEM_PROCESS_INFORMATION2_1*) data;
	...
	for (;;) {
		...
		spi = (SYSTEM_PROCESS_INFORMATION2_1*)((BYTE*)spi + offset);
	}

	SYSTEM_THREAD_INFORMATION_1*sti;
	SYSTEM_THREAD_INFORMATION_1*info = NULL;
	sti = (SYSTEM_THREAD_INFORMATION_1*)((BYTE*)spi + sizeof(*spi));
