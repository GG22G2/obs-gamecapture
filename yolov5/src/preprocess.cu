...
__global__ void warpAffine_kernel(
    ...
    //这是一个 bgr to rgb的例子，执行到这里需要保证c0 c1 c2 分别是 r g b 通道
//    float t = c2;
//    c2 = c0;
//    c0 = t;


    if constexpr (PIXEL_ORDER == BGR) {
        float t = c2;
        c2 = c0;
        c0 = t;
    }
...
void cuda_gpu_src_preprocess(
    ...
    AffineMatrix d2s = createAffineMatrix(src_width, src_height, dst_width, dst_height);
    ...
    if (pixelFormat == RGBA) {
        warpAffine_kernel<4, RGB><<<blocks, threads, 0, stream>>>(
                src, src_width * 4, src_width,
                src_height, dst, dst_width,
                dst_height, 128, d2s, jobs);
    } else if (pixelFormat == RGB) {
        warpAffine_kernel<3, RGB><<<blocks, threads, 0, stream>>>(
                src, src_width * 4, src_width,
                src_height, dst, dst_width,
                dst_height, 128, d2s, jobs);
    }else if (pixelFormat == BGRA) {
        warpAffine_kernel<4, BGR><<<blocks, threads, 0, stream>>>(
                src, src_width * 4, src_width,
                src_height, dst, dst_width,
                dst_height, 128, d2s, jobs);
    }else if (pixelFormat == BGR) {
        warpAffine_kernel<3, BGR><<<blocks, threads, 0, stream>>>(
                src, src_width * 4, src_width,
                src_height, dst, dst_width,
                dst_height, 128, d2s, jobs);
    }
...
//  这个输入图片必须是BGR格式
void cuda_memory_src_preprocess(
        uint8_t *src, int src_width, int src_height,
        float *dst, int dst_width, int dst_height,
        cudaStream_t stream) {
...
void cuda_batch_preprocess(std::vector<cv::Mat> &img_batch,
    ...
    for (size_t i = 0; i < img_batch.size(); i++) {
        cuda_memory_src_preprocess(img_batch[i].ptr(), img_batch[i].cols, img_batch[i].rows, &dst[dst_size * i],
                                   dst_width,
                                   dst_height, stream);
